...
class TestPromptAuditor:
    ...
    def test_init(self, config, mock_db_session):
        auditor = PromptAuditor(config, mock_db_session)
        ...
        assert auditor.db == mock_db_session

    def test_audit_disabled(self, config, mock_db_session):
    ...
    def test_audit_injection_detection(self, config, mock_db_session):
        auditor = PromptAuditor(config, mock_db_session)
    ...
    def test_audit_pii_masking(self, config, mock_db_session):
        auditor = PromptAuditor(config, mock_db_session)
    ...
    def test_audit_no_pii_no_injection(self, config, mock_db_session):
        auditor = PromptAuditor(config, mock_db_session)
    ...
    def test_invalid_regex_pattern(self, mock_db_session):
        config = {
            "enabled": True,
            "mask_pii": True,
            "block_injection": True,
